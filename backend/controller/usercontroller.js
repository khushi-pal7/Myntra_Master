...
exports.registermobile = A(async (req, res, next) => {
  ...
  sendMessage(options).then(response => {
    ...
    if (response.return === true) {
      ...
      console.log('SMS sending failed, falling back to console OTP')
...
exports.resendotp = A(async (req, res, next)=>{
  ...
  sendMessage(options).then(response => {
    if (response.return === true) {
      ...
      res.status(200).json({
        success: true,
        message: `OTP Sent on ${req.params.id} Successfully`
      })
     
    } else {
      console.log('SMS resend failed, falling back to console OTP')
